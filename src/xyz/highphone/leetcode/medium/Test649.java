...
public class Test649 {
    ...
    public static void main(String[] args) {
        ...
        System.out.println(new Test649().predictPartyVictory(senate));
    }

    public String predictPartyVictory(String senate) {
        assert senate != null && senate.length() > 0;
        int rCnt = 0;
        int dCnt = 0;
        ...
        while (true)
        {
            for (int i = 0; i < senates.length; i++)
            {
                ...
                if(senates[i] == 'R')
                {
                    ...
                    if(rNegative ==0)
                    {
                        dNegative ++;
                        dTotalN++;
                        if(dTotalN == dCnt && !isFirstLoop) return "Radiant";
                    }
                    else
                    {
                        rNegative--;
                    ...
                    if(dNegative == 0)
                    {
                        rNegative++;
                        rTotalN++;
                        if(rTotalN == rCnt && !isFirstLoop) return "Dire";
                    }
                    else
                    {
                        dNegative--;
            ...
            isFirstLoop = false;
            if(rTotalN > rCnt) return "Dire";
            if(dTotalN > dCnt) return "Radiant";
        }
